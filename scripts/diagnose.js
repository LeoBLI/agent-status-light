...
async function main() {
  ...
  const diagnostics = await getJson("/diagnostics").catch((error) => ({ error }));
  ...
  if (diagnostics.autoTransitions) {
    console.log(
      `Done display: ${
        diagnostics.autoTransitions.doneDisplayMs ?? diagnostics.autoTransitions.doneToIdleMs
      }ms`
    );
  ...
  if (hookHealth) {
    console.log(
      `Last hook event: ${
        hookHealth.lastHookEventAt ? formatDate(hookHealth.lastHookEventAt) : "never"
      }`
    );
    console.log(`Hook recently active: ${hookHealth.isHookRecentlyActive ? "yes" : "no"}`);
  } else {
  ...
  console.log("\nRecommendation:");
  for (const line of recommendations(serviceOk, diagnosticsOk, diagnostics, hookHealth)) {
    console.log(`- ${line}`);
  }
}
...
function getJson(route) {
  return new Promise((resolve, reject) => {
    const request = http.get(`${baseUrl}${route}`, { timeout: 2000 }, (response) => {
      const chunks = [];
      response.on("data", (chunk) => chunks.push(chunk));
      response.on("end", () => {
        const body = Buffer.concat(chunks).toString("utf8");
        if (!response.statusCode || response.statusCode < 200 || response.statusCode >= 300) {
          reject(new Error(`${route} returned HTTP ${response.statusCode}: ${body}`));
          return;
        }
    ...
    request.on("timeout", () => request.destroy(new Error(`${route} timed out`)));
...
function recommendations(serviceOk, diagnosticsOk, diagnostics, hookHealth) {
  if (!serviceOk) {
    return ["Start the app with npm run dev, or start only the HTTP service with npm run dev:server."];
  }

  if (!diagnosticsOk) {
    return ["The service is running, but /diagnostics is unavailable. Rebuild and restart the app."];
  }
  ...
  if (hookHealth?.isHookRecentlyActive) {
    ...
    lines.push("If Codex Desktop does not trigger hooks, use CLI mode or enable a fallback monitor later.");
    return lines;
  }
